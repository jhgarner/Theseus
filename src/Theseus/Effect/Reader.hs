...
data Reader r m a where
  ...
  Local :: (Reader r `Member` es, ef Identity) => (r -> r) -> Eff ef (Reader r : es) a -> Reader r (Eff ef es) a

ask :: Reader r `Member` es => Eff ef es r
ask = send Ask

local :: (Reader r `Member` es, ef Identity) => (r -> r) -> Eff ef (Reader r : es) a -> Eff ef es a
...
elabReader r (Local f m) next = runReaderId r $ next $ runIdentity <$> runReaderLocal f m

-- elabReader can also be written using interpose. The problem with interpose is
-- that goes away if the computation yields which seems extremely confusing. Is
-- there a better way to define interpose so that it doesn't get lost?
-- elabReader r (Local f m) next = runReaderId r $ next $ runIdentity <$> interpose (pure . pure) (elabReader $ f r) m

runReaderLocal :: (Reader r `Member` es, ef Identity) => (r -> r) -> Eff ef (Reader r : es) a -> Eff ef es (Identity a)
runReaderLocal r = handle (pure . pure) (elabReaderLocal r)

elabReaderLocal :: Reader r `Member` es => (r -> r) -> Handler (Reader r) ef es Identity
elabReaderLocal f Ask next = do
  r <- asks f
  runReaderLocal f $ next $ pure r
elabReaderLocal f (Local f' m) next = runReaderLocal f $ next $ runIdentity <$> runReaderLocal f' m
...
runReaderNoLocal r = fmap runIdentity . runReaderId r

runReaderNoLocalId :: ef Identity => r -> Eff ef (Reader r : es) a -> Eff ef es (Identity a)
runReaderNoLocalId r = handle (pure . pure) (elabReader r)

elabReaderNoLocal :: r -> Handler (Reader r) ef es Identity
elabReaderNoLocal r Ask next = runReaderId r $ next $ pure r
elabReaderNoLocal r (Local _ m) next = runReaderId r $ next $ runIdentity <$> runReaderLocal id m
