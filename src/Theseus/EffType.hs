...
data Freer :: Out -> [Effect] -> Type -> Type where
  ...
  Impure ::
    ...
    (forall g eff someEf. (forall a. ef a => someEf a, ControlFlow g someEf) => eff `IsMember` es -> g eff (Eff efSend esSend) x -> g eff (Eff ef es) a) ->
    Freer ef es a
...
instance Applicative (Freer ef es) where
  ...
  Impure eff next <*> m = Impure eff \member x -> next member x `cfApply` Eff (const m)

instance Monad (Freer ef es) where
  Pure ma >>= fmb = fmb ma
  Impure eff next >>= fmb = Impure eff \member x -> next member x `cfBind` (Eff . const . fmb)

lift :: Union es (FlipMember es) (Eff ef es) a -> Freer ef es a
lift f = Impure f \_ x -> x

raise :: forall eff ef es a. Eff ef es a -> Eff ef (eff : es) a
raise (Eff act) = Eff \Facts -> case act Facts of
  Pure a -> pure a
  Impure eff next -> Impure (raiseUnion eff) $ \case
